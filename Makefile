...
FASTRTPSGEN_JAR := artifacts/share/fastrtps/fastrtpsgen.jar
.PHONY : arm build
...
${FASTRTPSGEN_JAR} : 
	mkdir -p artifacts/share/fastrtps
	git submodule update --init --recursive
	docker run --rm -v $(shell pwd):/build -w /build --name gradle gradle:3.5-jdk7-alpine /bin/sh -c "cd /build/fastrtpsgen && gradle jar && cp /build/fastrtpsgen/share/fastrtps/fastrtpsgen.jar /build/artifacts/share/fastrtps/fastrtpsgen.jar"
